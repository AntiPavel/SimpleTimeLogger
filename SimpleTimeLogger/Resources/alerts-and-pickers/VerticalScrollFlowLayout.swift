//
//  VerticalScrollFlowLayout.swift
...
open class VerticalScrollFlowLayout: UICollectionViewFlowLayout {
    ...
    override open func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        dynamicItems?.forEach { item in
            ...
            transformItemIfNeeded(yPosition: convertedY, item: item)
        }
        return dynamicItems
    }
    
    private func transformItemIfNeeded(yPosition: CGFloat, item: UICollectionViewLayoutAttributes) {
        guard itemSize.height > 0, yPosition < itemSize.height * 0.5 else {
            return
        }
        
        let scaleFactor: CGFloat = scaleDistributor(xPosition: yPosition)
        
        let yDelta = getYDelta(yPosition: yPosition)
        ...
        item.alpha = alphaDistributor(xPosition: yPosition)
    ...
    override open func shouldInvalidateLayout(forBoundsChange newBounds: CGRect) -> Bool {
        ...
        guard let touchLocation = collectionView?.panGestureRecognizer.location(in: collectionView) else {
            return false
        }
        
        dynamicAnimator.behaviors.compactMap { $0 as? UIAttachmentBehavior }.forEach { behavior in
            guard let attrs = behavior.items.first as? UICollectionViewLayoutAttributes else { return }
    ...
    private func getUpdatedBehaviorItemCenter(behavior: UIAttachmentBehavior, touchLocation: CGPoint) -> CGPoint {
        ...
        guard let attrs = behavior.items.first as? UICollectionViewLayoutAttributes else {
     ...
     */
    private func distributor(xPosition: CGFloat, threshold: CGFloat, xOrigin: CGFloat) -> CGFloat {
        guard threshold > xOrigin else {
            return 1
        }
        var arg = (xPosition - xOrigin)/(threshold - xOrigin)
        arg = arg <= 0 ? 0 : arg
        let ySqrt = sqrt(arg)
        return ySqrt > 1 ? 1 : ySqrt
    }
    
    private func scaleDistributor(xPosition: CGFloat) -> CGFloat {
        return distributor(xPosition: xPosition, threshold: itemSize.height * 0.5, xOrigin: -itemSize.height * 5)
    }
    
    private func alphaDistributor(xPosition: CGFloat) -> CGFloat {
        return distributor(xPosition: xPosition, threshold: itemSize.height * 0.5, xOrigin: -itemSize.height)
    }
    
    private func getYDelta(yPosition: CGFloat) -> CGFloat {
        return itemSize.height * 0.5 - yPosition
    }
}
